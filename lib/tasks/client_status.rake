namespace :client_status do
  ...
  task correct: :environment do
    ...
    Organization.pluck(:short_name).each do |short_name|
      next if short_name == 'shared' || short_name != 'scc'
      ...
      Client.joins(client_enrollments: :leave_program).distinct.where("clients.status = ? AND (SELECT COUNT(*) FROM client_enrollments WHERE client_enrollments.status = ? AND client_enrollments.client_id = clients.id) = 0", 'Active', 'Active').distinct.each do |client|
