namespace :service_delivery do
  ...
  task create: :environment do
    ...
    workbook.sheets.each do |sheet_name|
      service = ServiceDelivery.find_or_create_by(name: sheet_name)
      ...
      column_values.each do |values|
        next if values.nil? || values[1..-1].nil?
        sub_cat_service = ServiceDelivery.find_or_create_by(name: values[0], parent_id: service.id)
        values[1..-1].each do |value|
          next if value.blank?
          child = ServiceDelivery.find_or_create_by(name: value, parent_id: sub_cat_service.id)
