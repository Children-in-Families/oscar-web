namespace :tenant_data do
  ...
  task :clean, [:short_name] => :environment do |task, args|
    ...
    unless Rails.env.production?
      ...
      system("PGPASSWORD=#{ENV['DATABASE_PASSWORD']} psql #{ENV['DATABASE_NAME']} -U #{ENV['DATABASE_USER']} -h #{ENV['DATABASE_HOST']} -p #{ENV['DATABASE_PORT']} < #{short_name}_production_2019_11_26.dump")
