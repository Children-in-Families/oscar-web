...
const CallForms = (props) => {
  ...
  const [caseActionNotRequired, setCaseActionNotRequiredModalOpen] = useState(
    false
  );
  ...
  const tabs = [
    ...
    {
      text: T.translate("newCall.index.tabs.client_referral_question"),
      step: 3,
    },
  ...
  const onChange = (obj, field) => (event) => {
    ...
    switch (obj) {
      case "call":
        ...
        break;
      case "client":
        if (event.type === "newObject") setClientData([...clientData, {}]);
        else setClientData(field);
        break;
      case "referee":
  ...
  const handleValidation = () => {
    ...
    const components = [
      ...
      { step: 2, data: clientData, fields: ["gender", "user_ids"] },
      {
        step: 3,
        data: clientData,
        fields: counselling ? ["phone_counselling_summary"] : [],
      },
  ...
  const handleTab = (goingToStep) => {
    ...
    if (
      ...
      if (step === 1) checkCallType()(() => setStep(goingToStep));
      else if (step === 2) checkClientExist()(() => setStep(goingToStep));
      else setStep(goingToStep);
  };

  const buttonNext = () => {
    if (handleValidation()) {
      if (step === 1) checkCallType()(() => setStep(step + 1));
      else if (step === 2) {
  ...
  const checkCallType = () => (callback) => {
    if (callData.call_type === "New Referral: Case Action NOT Required") {
      ...
      callData.call_type === "Seeking Information" ||
      callData.call_type === "Spam Call" ||
      callData.call_type === "Wrong Number"
    ) {
  ...
  const checkClientExist = () => (callback) => {
    ...
    if (
      clientData[0].outside === undefined ||
      clientData[0].outside === false
    ) {
      if (
        data.given_name !== "" ||
        data.family_name !== "" ||
  ...
  const handleSave = () => {
    if (handleValidation()) {
      ...
      if (refereeData.requested_update === false) setTaskData({});

      setOnSave(true);
      const action = callData.id ? "PUT" : "POST";
      const url = callData.id
        ? `/api/v1/calls/${callData.id}`
        : "/api/v1/calls";
      const message = T.translate(
        "newCall.index.message.call_has_been_created"
      );
      ...
      if (
        callData.call_type === "New Referral: Case Action Required" ||
        callData.call_type === "New Referral: Case Action NOT Required" ||
        callData.call_type === "Phone Counselling"
      ) {
      ...
      $.ajax({
        ...
        .fail((error) => {
          ...
          if (fieldErrors.kid_id) setErrorSteps([3]);
        });
    }
  };
      ...
      <>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          ...
          <button
            ...
            onClick={() => setDupClientModalOpen(false)}
          >
  ...
  const handleCheckValue = (object) => {
    ...
    if (object.concern_is_outside === undefined) return;
  ...
  const handleCancel = () => {
    const clientLocation = `/calls/${callData.id || ""}${
      ...
      <>
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <button
            style={{ margin: 5 }}
            className="btn btn-default"
            onClick={() => setCaseActionNotRequiredModalOpen(false)}
          >
            {T.translate("newCall.caseActionNotRequiredModalFooter.go_back")}
          </button>
          <button
            style={{ margin: 5 }}
            className="btn btn-primary"
            onClick={() => (
              setCaseActionNotRequiredModalOpen(false), setStep(step + 1)
            )}
          >
            {T.translate("newCall.caseActionNotRequiredModalFooter.iam_sure")}
          </button>
        </div>
      </>
    );
  ...
  return (
    <div className="containerClass">
      ...
      <Loading
        ...
        text={T.translate("index.duplicate_checker")}
      />
      ...
      <Modal
        ...
        content={
          <ProvidingUpdate
            ...
            onChange={onChange}
            onSave={() => {
              setProvidingUpdateModalOpen(false);
              handleSave();
            }}
            closeAction={() => setProvidingUpdateModalOpen(false)}
          />
      ...
      />
      <Modal
        title={`${T.translate("newCall.admin.save_call_as")} ${T.translate(
          "detailCall.call." + callData.call_type
        )}?`}
        ...
        content={
          <NoClientAttachedModal
            ...
            onChange={onChange}
            onSave={() => {
              setNoClientAttachedModalOpen(false);
              handleSave();
            }}
            closeAction={() => setNoClientAttachedModalOpen(false)}
          />
      ...
      <div className="actionfooter" style={{ paddingTop: "15px" }}>
        <div className="leftWrapper">
          <span className="btn btn-default" onClick={handleCancel}>
            {T.translate("newCall.index.cancel")}
          </span>
        </div>
        ...
        <div className="rightWrapper">
          ...
          >
            {T.translate("newCall.index.previous")}
          </span>
          {step !== 4 && (
            <span className={"clientButton allowButton"} onClick={buttonNext}>
              {T.translate("newCall.index.next")}
            </span>
          )}
          ...
          {step === 4 && (
            <span
              ...
              onClick={handleSave}
            >
              {T.translate("newCall.index.save")}
            </span>
          )}
        </div>
      </div>
