...
export default (props) => {
  ...
  const refereeRelationshipOpts = refereeRelationships.map((relationship) => ({
    label: T.translate("refereeRelationShip." + relationship.label),
  ...
  const [confirmRemoveClient, setConfirmRemoveClientModalOpen] =
    useState(false);
  ...
  const setAddressOptions = (obj, type) => {
    if (type === "concern_address") {
      ...
      if (
        obj.concern_province_id !== null &&
        obj.concern_province_id !== undefined
      )
        fetchData(
          "provinces",
          obj.concern_province_id,
          "districts",
          concernDataState
        );
      if (
        obj.concern_district_id !== null &&
        obj.concern_district_id !== undefined
      )
        fetchData(
          "districts",
          obj.concern_district_id,
          "communes",
          concernDataState
        );
      if (
        obj.concern_commune_id !== null &&
        obj.concern_commune_id !== undefined
      )
        fetchData(
          "communes",
          obj.concern_commune_id,
          "villages",
          concernDataState
        );
    } else {
  ...
  useEffect(() => {
    if (currentClient.concern_same_as_client) {
      const {
        ...
        concern_house,
        concern_address,
        concern_address_type,
        concern_outside_address,
        outside,
        province_id,
        district_id,
        commune_id,
      ...
      if (!same) onCheckSameAsClient({ data: concern_same_as_client });
    }
  }, [currentClient]);
  ...
  const onCheckSameAsClient = (data) => {
    ...
    if (same) setAddressOptions(currentClient, "concern_address");
    else resetAddressOptions("concern_address");
    ...
    modifyClientObject(clientIndex, {
      ...fields,
      concern_same_as_client: data.data,
    });
  };
  ...
  const onRelationshipChange = (event) => {
    ...
    if (isSelf) setAddressOptions(refereeObj, "address");
    else if (previousSelect === "self") resetAddressOptions("address");
    ...
    const fields = {
      outside: isSelf
        ...
        : previousSelect === "self"
        ? false
        : currentClient.outside,
      province_id: isSelf
        ? refereeObj.province_id
        : previousSelect === "self"
        ? null
        : currentClient.province_id,
      district_id: isSelf
        ? refereeObj.district_id
        : previousSelect === "self"
        ? null
        : currentClient.district_id,
      commune_id: isSelf
        ? refereeObj.commune_id
        : previousSelect === "self"
        ? null
        : currentClient.commune_id,
      village_id: isSelf
        ? refereeObj.village_id
        : previousSelect === "self"
        ? null
        : currentClient.village_id,
      street_number: isSelf
        ? refereeObj.street_number
        : previousSelect === "self"
        ? ""
        : currentClient.street_number,
      house_number: isSelf
        ? refereeObj.house_number
        : previousSelect === "self"
        ? ""
        : currentClient.house_number,
      current_address: isSelf
        ? refereeObj.current_address
        : previousSelect === "self"
        ? ""
        : currentClient.current_address,
      address_type: isSelf
        ? refereeObj.address_type
        : previousSelect === "self"
        ? ""
        : currentClient.address_type,
      outside_address: isSelf
        ? refereeObj.outside_address
        : previousSelect === "self"
        ? ""
        : currentClient.outside_address,
    };

    modifyClientObject(clientIndex, {
      ...fields,
      referee_relationship: event.data,
    });
  };
  ...
  const renderClientNavigation = () => {
    if (clients.length > 1)
      return clients.map((client, index) => (
        <button
          className={`btn ${
            clientIndex === index ? "btn-primary" : "btn-default"
          } `}
  ...
  const hanldeOnChangeAddressInputs = (obj, field) => (event) => {
    if (typeof field === "object") modifyClientObject(clientIndex, field);
    else {
  ...
  return (
    <div className="containerClass">
      <Modal
        ...
        content={
          <ConfirmRemoveClientModal
            data={{ T }}
            onClick={() => {
              setConfirmRemoveClientModalOpen(false);
              removeClient();
            }}
            closeAction={() => setConfirmRemoveClientModalOpen(false)}
          />
        ...
        <div className="row">
          ...
          <div
            className={`col-xs-12 col-md-6 col-lg-4 ${
              call.call_type === "Phone Counselling" ? "hidden" : ""
            }`}
          >
        ...
        <div className="col-xs-12 col-md-6 col-lg-3">
          <SelectInput
            ...
            isError={
              ...
              (currentClient.gender === undefined ||
      ...
      <div className="row">
        ...
        <div className="col-xs-12 col-md-6 col-lg-3">
          <SelectInput
            ...
            required
            isError={
              (errorFields.includes("user_ids") &&
                currentClient.user_ids !== undefined &&
                currentClient.user_ids.length < 1) ||
              currentClient.user_ids === undefined ||
              currentClient.user_ids === null
            }
      ...
      <Address
        disabled={currentClient.referee_relationship === "self"}
        outside={currentClient.outside || false}
      ...
      <div className="row">
        <div className="col-xs-12 col-md-6">
          <div className="row">
            <div className="col-xs-12 col-md-6">
              <TextInput
                label={T.translate("newCall.referralInfo.client_phone")}
                type="text"
                onChange={handleOnChangeText("client_phone")}
                value={currentClient.client_phone}
              />
            </div>
            <div className="col-xs-12 col-md-6">
              <SelectInput
                ...
                onChange={handleOnChangeOther("phone_owner")}
                value={currentClient.phone_owner}
              />
            </div>
            <div className="col-xs-12 col-md-6">
              <TextInput
                label={T.translate("newCall.referralInfo.client_email")}
                onChange={handleOnChangeText("client_email")}
                value={currentClient.client_email}
              />
            </div>
            <div className="col-xs-12 col-md-6">
              <SelectInput
                ...
                options={phoneEmailOwnerOpts}
                onChange={handleOnChangeOther("email_owner")}
                value={currentClient.email_owner}
              />
            </div>
          </div>
        </div>
        <div
          className={
            "col-xs-12 col-md-6" + (currentClient.outside ? " hidden" : "")
          }
        >
          <TextArea
      ...
      >
        <button
          ...
          style={{ margin: 5 }}
          onClick={() => {
            onChange("client", {})({ type: "newObject" });
            setClientIndex(clients.length);
          }}
        >
          {T.translate("newCall.referralInfo.add_another_client")}
        </button>
        {clients.length > 1 && (
          <button
            className="btn btn-danger"
            style={{ margin: 5 }}
            onClick={() => setConfirmRemoveClientModalOpen(true)}
          >
            {T.translate("newCall.referralInfo.remove_client")}
          </button>
        )}
      </div>
        ...
        <>
            ...
            <div className="row">
              ...
              <div className="col-xs-12 col-md-6 col-lg-3">
                <Checkbox
                  ...
                  checked={currentClient.concern_same_as_client}
          ...
          <div className="row">
            ...
            <div
              className={
                "col-xs-12 col-md-6" +
                (clients[0].concern_is_outside ? " hidden" : "")
              }
            >
              <TextArea
