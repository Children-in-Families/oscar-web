module FormBuilderAttachmentsHelper
  def form_buildable_path(resource, index, label, params_program_stream)
    if controller_name == 'leave_programs' || controller_name == 'leave_enrolled_programs'
      link_to polymorphic_path([@client || resource.client_enrollment&.client, resource.client_enrollment, resource], program_stream_id: @program_stream.id, file_index: index, file_name: label, program_streams: params_program_stream), method: :delete, data: { confirm: t('.are_you_sure') }, class: "delete btn btn-outline btn-danger #{disabled?}" do
        fa_icon('trash')
      end
