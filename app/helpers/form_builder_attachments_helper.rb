module FormBuilderAttachmentsHelper
  def form_buildable_path(resource, index, label, params_program_stream)
    if controller_name == 'leave_programs' || controller_name == 'leave_enrolled_programs'
    ...
    elsif controller_name == 'client_enrollments'
      ...
      link_to polymorphic_path([@custom_formable, resource], custom_field_id: @custom_field.id, file_index: index, file_name: label), method: :delete, data: { confirm: t('.are_you_sure') }, class: 'delete btn btn-outline btn-danger' do
        fa_icon('trash')
      end
    end
  end
