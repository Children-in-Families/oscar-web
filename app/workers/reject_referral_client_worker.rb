class RejectReferralClientWorker
  ...
  def perform(user_name, referred_to, referral_id)
    referral = Referral.find referral_id
    Organization.switch_to referral.referred_from

    user = User.find referral.referee_id
    RejectReferralClientMailer.client_rejection_email(user_name, referred_to, referral).deliver_now if user.present?
  end
end
