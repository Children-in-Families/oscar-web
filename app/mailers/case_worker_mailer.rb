class CaseWorkerMailer < ApplicationMailer
  ...
  def overdue_tasks_notify(user, short_name)
    @user = user
    @overdue_tasks = user.tasks.exclude_exited_ngo_clients.where(client_id: user.clients.all_active_types_and_referred_accepted.ids).overdue_incomplete_ordered
