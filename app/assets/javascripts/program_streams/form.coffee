        ...
        $(labelFields).each (index, label) ->
          if (numberOfField == 1) && (label.textContent == labelText)
            ...
            unless $(label).parent().find('label.error').is(':visible')
              $(label).parent().append('<label class="error">This field is duplicated.</label>')
