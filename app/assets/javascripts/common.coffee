CIF.Common =
  ...
  checkValidationErrorExistOnSaving: ->
    setTimeout (->
      $fileInputs = $('.file-caption-main input')
      $.each $fileInputs, (_index, fileInput) ->
        $fileInput = $(fileInput)
        $fileInputWrapper = $fileInput.closest('.form-group.file')
        imagePath = $fileInput.attr('value')

        if !!imagePath
          $fileInputWrapper.find('.file-input.theme-explorer').removeClass('file-input-new')
          ...
          $fileInputWrapper.find('.file-preview table > tbody.file-preview-thumbnails').append(imagePreview)
    ), 1000
