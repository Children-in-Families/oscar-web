  ...
  class Import
    ...
    def clients
      (workbook_second_row..workbook.last_row).each do |row_index|
        ...
        new_client['initial_referral_date'] = workbook.row(row_index)[headers['* Initial Referral Date']].to_s&.presence || '2020-09-01'
        ...
        case_worker_ids = []
        if case_worker_name[/\//]
          case_worker1, case_worker2 = case_worker_name.split('/')
          [case_worker1, case_worker2].each do |case_worker|
            case_worker_by_name = case_worker.split(' ')
            case_worker_by_attr = case_worker_by_name.is_a?(String) ? { first_name: case_worker_by_name.last } : { first_name: case_worker_by_name.last.squish, last_name: case_worker_by_name.first.squish }
            new_client['user_id'] = User.find_by(first_name: case_worker_by_attr[:first_name]).try(:id)
            case_worker_ids << new_client['user_id']
            next if new_client['user_ids'].present?

            case_worker_by_name = case_worker.split(' ')
            case_worker_by_attr = case_worker_by_name.is_a?(String) ? { first_name: case_worker_by_name.last } : { first_name: case_worker_by_name.last.squish, last_name: case_worker_by_name.first.squish }
            new_client['user_id'] = create_user_received_by(case_worker_by_attr)
            case_worker_ids << create_user_received_by(case_worker_by_attr)
          end
          new_client['user_ids'] = case_worker_ids.compact.uniq
        else
          new_client['user_id'] = User.find_by(first_name: case_worker_name.split(' ').last).try(:id)
          new_client['user_ids'] = [new_client['user_id']]
          if new_client['user_ids'].compact.blank?
            case_worker_by_name = case_worker_name.split(' ')
            case_worker_by_attr = case_worker_by_name.is_a?(String) ? { first_name: case_worker_by_name.last } : { first_name: case_worker_by_name.last.squish, last_name: case_worker_by_name.first.squish }
            new_client['user_ids'] = [create_user_received_by(case_worker_by_attr)]
          end
        end
    ...
    def create_user_received_by(attributes)
      ...
      user = User.find_or_create_by(first_name: attribute['first_name']) do |user|
                ...
                user.email = "#{attribute['first_name']}@gardenofhope.com"
