  ...
  def show
    respond_to do |format|
      format.html do
        ...
        @internal_referrals = @client.internal_referrals.joins(:program_streams).select('DISTINCT ON (internal_referrals.id, program_streams.id) internal_referrals.id, internal_referrals.client_id, program_streams.name program_name, internal_referrals.created_at')
      end
      format.pdf do
        form        = params[:form]
