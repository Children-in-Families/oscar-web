class ProgramStreamsController < AdminController
  ...
  def program_stream_params
    ...
    default_params = [:name, :rules, :description, :enrollment, :exit_program, :tracking, :quantity, program_exclusive: [], mutual_dependence: [], domain_ids: []]
  ...
  def program_without_tracking?
    params[:program_stream][:tracking].present?
  end
