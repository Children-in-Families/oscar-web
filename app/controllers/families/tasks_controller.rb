class Families::TasksController < AdminController
  load_and_authorize_resource
  before_action :find_family
  before_action :authorize_family, only: %i[new create]
  ...
  def create
    ...
    respond_to do |format|
      if @task.save
        ...
        format.html { redirect_to family_tasks_path(@family), notice: t('.successfully_created') }
      else
  ...
  def update
    ...
    if @task.update_attributes(task_params)
      ...
      redirect_to family_tasks_path(@family), notice: t('.successfully_updated')
    else
      render :edit
    end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      if @task.destroy
        ...
        format.html { redirect_to family_tasks_path(@family), notice: t('.successfully_deleted') }
      else
        format.json { render json: @task.errors, status: 422 }
        format.html { redirect_to family_tasks_path(@family), alert: t('.failed_delete') }
      end
    end
  end
  ...
  def task_params
    params.require(:task).permit(:domain_id, :name, :completion_date, :remind_at, :relation, :case_note_id)
  end
