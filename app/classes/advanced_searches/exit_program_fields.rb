      ...
      program_streams.each do |program_stream|
        ...
        program_stream.exit_program.each do |json_field|
          ...
          if json_field['type'] == 'text' || json_field['type'] == 'textarea'
            @text_type_list << "exitprogram__#{program_stream.name}__#{json_field['name']}"
          elsif json_field['type'] == 'number'
            @number_type_list << "exitprogram__#{program_stream.name}__#{json_field['name']}"
          elsif json_field['type'] == 'date'
            @date_type_list << "exitprogram__#{program_stream.name}__#{json_field['name']}"
          elsif json_field['type'] == 'select' || json_field['type'] == 'checkbox-group' || json_field['type'] == 'radio-group'
            drop_list_values = []
            drop_list_values << "exitprogram__#{program_stream.name}__#{json_field['name']}"
