    ...
    def generate
      @basic_rules.each do |rule|
        ...
        if ASSOCIATION_FIELDS.include?(field)
          association_filter = AdvancedSearches::FamilyAssociationFilter.new(@families, field, operator, value).get_sql
          ...
          custom_field = AdvancedSearches::FamilyCustomFormSqlBuilder.new(custom_form, rule).get_sql
