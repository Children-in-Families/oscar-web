module AdvancedSearches
  module Families
    class FamilyCustomFormSqlBuilder

      def initialize(selected_custom_form, rule)
        @selected_custom_form = selected_custom_form
        field          = rule['field']
        @field         = field.split('_').last.gsub("'", "''").gsub('&quote;', '"').gsub('&', '&amp;').gsub('<', '&lt;').gsub('>', '&gt;')
      ...
      def get_sql
        ...
        custom_field_properties = CustomFieldProperty.where(custom_formable_type: 'Family', custom_field_id: @selected_custom_form)
        ...
        family_ids = properties_result.pluck(:custom_formable_id).uniq
