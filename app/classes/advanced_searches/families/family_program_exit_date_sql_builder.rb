  ...
  module Families
    class FamilyProgramExitDateSqlBuilder
      ...
      def get_sql
        ...
        leave_programs = LeaveProgram.joins(:family_enrollment).where(program_stream_id: @program_stream_id).distinct
        family_ids = []
        ...
        when 'greater_or_equal'
          leave_program_date = leave_programs.where('exit_date >= ?', @value)
        when 'is_empty'
          # family_ids = familyEnrollment.where(program_stream_id: @program_stream_id).select("family_id").group(:family_id).having("count(*) = 1 and max(status)= 'Active'")
          leave_program_date = leave_programs.where.not(exit_date: nil)
          family_ids         = leave_program_date.joins(:family_enrollment).pluck('family_enrollments.programmable_id').uniq
        ...
        family_ids = leave_program_date.pluck('family_enrollments.programmable_id')
