class ClientAdvancedFilterCustomFields
  ...
  def render
    number_fields       = @number_type_list.map { |item| AdvancedFilterTypes.number_options(item, item) }
    text_fields         = @text_type_list.map { |item| AdvancedFilterTypes.text_options(item, item) }
    date_picker_fields  = @date_type_list.map { |item| AdvancedFilterTypes.date_picker_options(item, item) }
    drop_list_fields    = @drop_down_type_list.map { |item| AdvancedFilterTypes.drop_list_options(item.first, item.first , item.last) }
