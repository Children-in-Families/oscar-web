class Client < ActiveRecord::Base
  ...
  def self.filter(options)
    ...
    query = query.joins(:province).where("provinces.name iLIKE ?", "%#{options[:current_province]}%")         if options[:current_province_id].present?
  ...
  def self.find_shared_client(options)
    ...
    shared_clients = shared_clients.where(birth_province_id: options[:birth_province_id]) if options[:birth_province_id].present?
    Organization.switch_to current_org
    shared_clients.pluck(:slug)
  end
