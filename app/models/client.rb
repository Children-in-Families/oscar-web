class Client < ActiveRecord::Base
  ...
  def next_appointment_date
    ...
    next_appointment_date = next_appointment.created_at + 1.month
  end
  ...
  def reset_user_to_tasks
    if self.user_id_changed?
      self.tasks.update_all(user_id: self.user_id)
    end
  end
  ...
  def age
    date_of_birth.present? ? (Date.today.year - date_of_birth.year) : ''
  end
