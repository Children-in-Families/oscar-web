class Partner < ActiveRecord::Base
  ...
  scope :organisation_type_is,       ->         { where.not(organisation_type: '').map{|partner| partner.organisation_type}.uniq }
  ...
  scope :province_is,                ->         { where.not(province_id: nil).map { |p| [p.province.name, p.province_id] if p.province }.uniq }
