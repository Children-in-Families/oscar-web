module UpdateFieldLabelsCustomForm
  def labels_update(new_fields, old_fields, objects)
    ...
    fields_changed.each do |field_changed|
      old_fields.each do |custom_form|
        if custom_form['name'] == field_changed['name']
          if custom_form['type'] == 'file'
            field_labels_changed << [custom_form['label'], field_changed['label']]
          else
            labels_changed << [custom_form['label'], field_changed['label']]
          end
        end
      end
    end
