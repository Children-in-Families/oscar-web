class AddUniqFielsToDomains < ActiveRecord::Migration[5.2]
  def change
    if schema_search_path =~ /^\"isf\"/
      reversible do |dir|
        dir.up do
          Domain.joins(:assessment_domains).where(custom_domain: true).order('domains.id').distinct.map{|domain| [domain.id, domain.name, domain.identity] }.group_by{|domain| "#{domain.second} #{domain.third}" }.each do |group|
            ...
            domain.update_column(:identity, "#{identity} 2")
