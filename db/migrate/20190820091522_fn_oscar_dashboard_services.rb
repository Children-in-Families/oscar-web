  ...
  def change
    reversible do |dir|
      dir.up do
        if schema_search_path =~ /^\"public\"/
          execute <<-SQL.squish
            ...
            GRANT EXECUTE ON FUNCTION "public"."fn_oscar_dashboard_services"(varchar) TO "#{ENV['POWER_BI_GROUP']}";
          SQL
        end
      end
      ...
      dir.down do
        if schema_search_path =~ /^\"public\"/
          execute <<-SQL.squish
            REVOKE EXECUTE ON FUNCTION "public"."fn_oscar_dashboard_services"(varchar) FROM "#{ENV['POWER_BI_GROUP']}";
            DROP FUNCTION IF EXISTS "public"."fn_oscar_dashboard_services"(varchar) CASCADE;
          SQL
        end
      end
    end
