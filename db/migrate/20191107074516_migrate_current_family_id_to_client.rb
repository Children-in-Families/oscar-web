class MigrateCurrentFamilyIdToClient < ActiveRecord::Migration
  def up
    values = Client.joins(:families).distinct.select('clients.id, (SELECT cases.family_id FROM cases WHERE cases.client_id = clients.id ORDER BY cases.created_at DESC LIMIT 1) ORDER BY cases.created_at DESC LIMIT 1) as family_id').pluck(:id, :family_id).to_h.map do |id, family_id|
      "(#{id}, #{family_id || 'NULL'})"
    end.join(", ")
