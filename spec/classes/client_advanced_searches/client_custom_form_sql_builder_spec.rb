describe AdvancedSearches::ClientCustomFormSqlBuilder, 'Method' do
  ...
  context '#generate' do
    it 'return client = query string' do
      ...
      filter_string = AdvancedSearches::ClientCustomFormSqlBuilder.new(custom_form.id, rules).get_sql
    ...
    it 'return client < query string' do
      ...
      filter_string = AdvancedSearches::ClientCustomFormSqlBuilder.new(custom_form, rules).get_sql
      expect(filter_string[:id]).to include 'clients.id IN (?)'
      expect(filter_string[:values]).to include(client.id)
    end
