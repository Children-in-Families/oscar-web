describe ClientEnrollmentHistory, 'class method' do
  ...
  xcontext 'initial(client_enrollment)' do
  ...
  xcontext 'format_property(attributes)' do
    ...
    let!(:program_stream) { FactoryBot.create(:program_stream, enrollment: enrollment.to_json) }
    let!(:client_enrollment_2){ FactoryBot.create(:client_enrollment, program_stream: program_stream, properties: {"Checkbox Group"=>["Option 1"], "Radio.Group"=>"Option 1", "Select"=>["Option 1"]}.to_json) }
