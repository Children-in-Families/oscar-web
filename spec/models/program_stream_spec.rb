...
describe ProgramStream, 'validate rules edition' do
  context 'ProgramStream has rules' do
    ...
    it 'unable to save program stream' do
      ...
      expect(program_stream.errors[:rules]).to include('Conditions cannot be changed or added because it breaks the previous conditions.')
    end
    ...
    it 'able to save program stream' do
      ...
      expect(program_stream.errors[:rules]).not_to include('Conditions cannot be changed or added because it breaks the previous conditions.')
    end
  end
...
describe ProgramStream, 'validate program edition' do
  context 'ProgramStream has no mutual dependence and program exclusive' do
    ...
    it 'unable to save program stream program exclusive' do
      ...
      expect(program_stream.errors[:program_exclusive]).to include('Program Exclusive cannot be changed or added because it breaks the previous conditions.')
    end

    it 'able to save program stream program exclusive' do
      program_stream.update(program_exclusive: [program_stream_2.id])
      expect(program_stream.errors[:program_exclusive]).not_to include('Program Exclusive cannot be changed or added because it breaks the previous conditions.')
    end
...
describe ProgramStream, 'validate presence of label field' do
  context 'Enrollment' do
    context 'valid' do
      ...
      valid_program_stream = ProgramStream.new(name: 'Test', enrollment: enrollment)
      it { expect(valid_program_stream.valid?).to be_truthy }
    end

    context 'invalid' do
      enrollment = [{"name"=>"date-1497520151012", "type"=>"date", "label"=>"", "className"=>"calendar"}]
      invalid_program_stream = ProgramStream.new(name: 'Test', enrollment: enrollment)
  ...
  context 'Leave Program' do
    context 'valid' do
      exit_program = [{"name"=>"date-1497520151012", "type"=>"date", "label"=>"Enrolment Date", "className"=>"calendar"}]
      valid_program_stream = ProgramStream.new(name: 'Test', exit_program: exit_program)
      it { expect(valid_program_stream.valid?).to be_truthy }
    end

    context 'invalid' do
      exit_program = [{"name"=>"date-1497520151012", "type"=>"date", "label"=>"", "className"=>"calendar"}]
      invalid_program_stream = ProgramStream.new(name: 'Test', exit_program: exit_program)
      invalid_program_stream.valid?
