describe EntityEnrollmentTrackingHistory, 'class methods' do
  before do
  ...
  context 'format_property(attributes)' do
    ...
    let!(:enrollment_tracking){ create(:enrollment_tracking, :with_history, tracking: tracking, properties: {"Checkbox Group"=>["Option 1"], "Radio.Group"=>"Option 1", "Select"=>["Option 1"]}.to_json) }
    it 'join string and dot with underscore and as lowercase' do
      expect(EntityEnrollmentTrackingHistory.where({'object.id' => enrollment_tracking.id}).first.object['properties']).to eq({"checkbox_group"=>["Option 1"], "radio_group"=>"Option 1", "select"=>["Option 1"]})
    end
  end
end
