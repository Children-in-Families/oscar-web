  ...
  describe 'PUT #update' do
    ...
    context 'when user loged in' do
      ...
      context 'when try to update case note with incompleted tasks' do
        before do
          case_note_params = { format: 'json', case_note: { meeting_date: Time.now, case_note_domain_groups_attributes: [{note: FFaker::Lorem.paragraph, domain_group_id: @domain_group.id}] } }
      ...
      context 'when try to update case note with completed task' do
        before do
          case_note_params = { format: 'json', case_note: { meeting_date: Time.now, case_note_domain_groups_attributes: [{note: FFaker::Lorem.paragraph, domain_group_id: @domain_group.id, task_ids: tasks.map(&:id)}] } }
          put "/api/v1/clients/#{client.id}/case_notes/#{case_note.id}", case_note_params, @auth_headers
        end
      ...
      context 'when try to update case note without domain group' do
        before do
          case_note_params = { format: 'json', case_note: { meeting_date: Time.now, case_note_domain_groups_attributes: [{note: FFaker::Lorem.paragraph}] } }
          put "/api/v1/clients/#{client.id}/case_notes/#{case_note.id}", case_note_params, @auth_headers
        end
