  ...
  describe 'POST #create' do
    ...
    context 'when user loged in' do
      ...
      context 'when try create assessment' do
        before do
          assessment = { format: 'json', assessment: { assessment_domains_attributes: [{ domain_id: domain.id, score: score, reason: FFaker::Lorem.paragraph}] } }
      ...
      context 'when try create assessment without domain' do
        before do
          assessment = { format: 'json', assessment: { assessment_domains_attributes: [{score: score, reason: FFaker::Lorem.paragraph}] } }
          post "/api/v1/clients/#{client.id}/assessments", assessment, @auth_headers
        end
